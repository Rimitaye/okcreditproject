...
import { 
  ...
  MdInfo, MdContactPage, MdChevronRight 
} from "react-icons/md";
...
export default function EditProfile() {
  ...
  // 1. Updated State for form inputs (Including all New Fields)
  const [formData, setFormData] = useState({
    name: localStorage.getItem("userName") || "",
    phone: localStorage.getItem("userPhone") || "",
    businessName: localStorage.getItem("businessName") || "",
    email: localStorage.getItem("userEmail") || "",
    aboutUs: localStorage.getItem("aboutUs") || "",
    contactPerson: localStorage.getItem("contactPerson") || "",
    address: localStorage.getItem("address") || "",
    businessType: localStorage.getItem("businessType") || "Retail Shop",
    category: localStorage.getItem("category") || "General Store"
  });
  ...
  const handleSave = async () => {
    try {
      ...
      if (res.ok) {
        // 🟢 Save all fields to LocalStorage for instant UI update
        localStorage.setItem("userName", formData.name);
        localStorage.setItem("userPhone", formData.phone);
        localStorage.setItem("businessName", formData.businessName);
        localStorage.setItem("userEmail", formData.email);
        localStorage.setItem("aboutUs", formData.aboutUs);
        localStorage.setItem("contactPerson", formData.contactPerson);
